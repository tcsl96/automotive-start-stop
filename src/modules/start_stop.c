...
{
    if (ss_s_system_input->ss_b_button_pressed == true)
    {
        ss_s_system_output->ss_b_start_stop_on = START_STOP_OFF;
    }
    else
    {
        ss_s_system_output->ss_b_hardware_ok = hardware_check_hardware(
        ...
        if (ss_s_system_output->ss_b_hardware_ok == false)
        {
            ss_s_system_output->ss_b_start_stop_on = START_STOP_OFF;
        }
        else
        {
            ss_s_system_output->ss_b_safety_ok = safety_check_safety(
            ...
            ss_s_system_output->ss_b_engine_temp_ok = power_check_engine_temp(ss_s_system_input->fc_tmp, FIXED7_9_FRAC);
            ss_s_system_output->ss_b_battery_ok = power_check_battery(ss_s_system_input->SOC, FIXED1_15_FRAC);
            ss_s_system_output->ss_b_speed_ok = dynamics_check_speed(ss_s_system_input->mpha, FIXED7_9_FRAC);
            ss_s_system_output->ss_b_brake_ok = dynamics_check_brake(ss_s_system_input->lim_clutch_dis, ss_s_system_input->time);
            ss_s_system_output->ss_b_air_cond_ok = wellbeing_check_air_cond(ss_s_system_input->ss_air_cond_speed, ss_s_system_input->SOC, FIXED1_15_FRAC);

            ss_s_system_output->ss_b_start_stop_on = (
                ss_s_system_output->ss_b_safety_ok &&
                ss_s_system_output->ss_b_engine_temp_ok &&
                ss_s_system_output->ss_b_battery_ok &&
                ss_s_system_output->ss_b_safety_ok &&
                ss_s_system_output->ss_b_brake_ok &&
                ss_s_system_output->ss_b_air_cond_ok
            );
        }
    }
}
...
void start_stop_check_latency(sys_in_t *ss_s_system_input, sys_out_t *ss_s_system_output)
{
    ...
    if ((ss_s_system_output->ss_b_start_stop_on || ss_s_system_input->ss_b_engine_on) == false)
    {
        ...
        {
            ss_s_system_output->ss_b_latency_alert = LATENCY_ON;
        }
        else
        {
            ss_s_system_output->ss_b_latency_alert = LATENCY_OFF;
        }
    }
    else
    {
